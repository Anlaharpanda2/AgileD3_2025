<template>
  ...
  <div class="w-full h-full flex flex-col bg-white">
    ...
    <div class="text-center p-6 border-b border-gray-200" data-aos="fade-up">
    ...
    <!-- Loading State -->
    <div v-if="loading" class="flex-grow flex flex-col items-center justify-center p-4 text-center">
    ...
    <!-- Error State -->
    <div v-else-if="error" class="flex-grow flex items-center justify-center p-4 text-center">
    ...
    <div v-else-if="announcements.length === 0" class="flex-grow flex items-center justify-center p-4 text-center">
      <div class="bg-gray-50 border border-gray-200 rounded-xl p-6 w-full max-w-sm">
        <!-- SVG Icon Inlined -->
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-10 h-10 text-gray-400 mx-auto mb-3 opacity-75">
          <path d="M20 17H4V16C4 12.6863 6.68629 10 10 10V9C10 6.23858 12.2386 4 15 4H16C18.7614 4 21 6.23858 21 9V16.9999L20 17ZM12 21C12.5523 21 13 20.5523 13 20H11C11 20.5523 11.4477 21 12 21Z"></path>
        </svg>
        <p class="font-medium text-gray-700">Belum Ada Pengumuman</p>
        <p class="text-sm text-gray-500 mt-1">Silakan periksa kembali nanti.</p>
      </div>
    </div>
        ...
        >
          <div class="flex items-start justify-between gap-4">
            ...
            <div class="flex-grow min-w-0">
              <h4 class="text-base font-semibold text-gray-800 leading-tight truncate">{{ announcement.judul }}</h4>
              <p class="text-sm text-gray-500 mt-1 line-clamp-2">{{ announcement.isi }}</p>
            </div>
            <div class="flex-shrink-0 text-right">
                <span :class="getTagClasses(announcement.created_at)">
                  {{ getAnnouncementLabel(announcement.created_at) }}
                </span>
                <p class="text-xs text-gray-400 mt-2">{{ formatDate(announcement.created_at) }}</p>
            </div>
          </div>
        </div>
      </div>
