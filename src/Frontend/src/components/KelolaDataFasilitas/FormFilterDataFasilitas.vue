        ...
        <div class="relative bg-gradient-to-r from-pink-50 to-rose-50 border-b border-gray-100">
          <div class="flex items-center justify-between p-6">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h2 class="text-xl font-bold text-gray-800">Filter Data Fasilitas</h2>
...
const filteredColumns = computed(() => {
  const excludedColumns = ['photo', 'deleted_at', 'photo_url', 'kode_fasilitas', 'jenis_fasilitas', 'tanggal_pengadaan', 'lokasi', 'kondisi'];
...
const getColumnIcon = (column: string): string => {
  ...
  // Data fasilitas
  if (col.includes('nama_fasilitas')) return 'fas fa-building';
  if (col.includes('deskripsi')) return 'fas fa-info-circle';
  if (col.includes('kategori')) return 'fas fa-tags';
  if (col.includes('jumlah')) return 'fas fa-calculator';
  if (col.includes('status')) return 'fas fa-check-circle';
