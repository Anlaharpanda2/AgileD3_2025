 ...
 px-6 py-4 sm:px-8 sm:py-6 rounded-t-xl">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h2 class="text-xl sm:text-2xl font-bold text-white">Edit Data Panitia</h2>
                <p class="text-pink-100 text-xs sm:text-sm">Perbarui informasi panitia</p>
              </div>
            </div>
            <button
              @click="$emit('close')"
...
const handleFileUpload = (event: Event) => {
  ...
  if (target.files && target.files[0]) {
    ...
    form.current_foto_url = URL.createObjectURL(target.files[0]); // Tampilkan preview foto baru
  }
};
...
const submitForm = () => {
  formRef.value.validate(async (valid: boolean) => {
    ...
    try {
      const response = await api.post(`/kelola/panitia/${form.id}`, formData, {
        headers: {
      ...
      ElNotification({
        title: 'Berhasil',
        message: response.message || 'Data berhasil diupdate!',
        type: 'success',
        duration: 3000,
      });
      emit('close');
