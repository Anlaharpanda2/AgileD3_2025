 ...
 px-6 py-4 sm:px-8 sm:py-6 rounded-t-xl">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h2 class="text-xl sm:text-2xl font-bold text-white">Tambah Data Panitia</h2>
                <p class="text-pink-100 text-xs sm:text-sm">Isi form untuk menambahkan data panitia baru</p>
              </div>
            </div>
            <button
              @click="$emit('close')"
          ...
          >
            ...
            <div class="bg-gray-50 rounded-xl p-5 sm:p-6 border border-gray-100 shadow-sm">
              <div class="flex items-center space-x-3 mb-5">
                ...
                <h3 class="text-base sm:text-lg font-semibold text-gray-800">Informasi Panitia</h3>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <el-form-item label="Nama Panitia" prop="nama_panitia">
                  <el-input
                    v-model="form.nama_panitia"
                ...
                <el-form-item label="Email" prop="email">
                  <el-input
                    v-model="form.email"
                    placeholder="Masukkan email panitia"
                    size="large"
                    class="modern-input"
                  >
                    <template #prefix>
                      <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m-4.5-1.5a2.5 2.5 0 01-5 0V12a9 9 0 109 9"/>
                      </svg>
                    </template>
                  </el-input>
                </el-form-item>
                ...
                <el-form-item label="Foto" prop="foto">
                  <input type="file" @change="handleFileUpload" accept="image/*" class="modern-input"/>
                  <div v-if="form.foto_preview_url" class="mt-2">
                    <p class="text-sm text-gray-500">Pratinjau Foto:</p>
                    <img :src="form.foto_preview_url" alt="Pratinjau Foto Panitia" class="w-24 h-24 object-cover rounded-full"/>
                  </div>
                </el-form-item>
              </div>
            </div>
...
const rules = {
  ...
  nama_panitia: [{ required: true, message: 'Nama panitia wajib diisi', trigger: 'blur' }],
...
const handleFileUpload = (event: Event) => {
  ...
  if (target.files && target.files[0]) {
    ...
    form.foto_preview_url = URL.createObjectURL(target.files[0]); // Tampilkan preview foto baru
  }
};
...
const submitForm = () => {
  formRef.value.validate(async (valid: boolean) => {
    ...
    try {
      const response = await api.post('/kelola/panitia', formData, {
        headers: {
