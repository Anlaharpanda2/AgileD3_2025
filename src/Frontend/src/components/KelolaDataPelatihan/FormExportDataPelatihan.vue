...
// Fungsi untuk mengekspor data ke format Excel menggunakan ExcelJS
async function ExportExcel() {
  ...
  worksheet.columns = exportColumns.map(col => ({
    header: col.label,
    key: col.key,
    width: Math.max(col.label.length, 15) // Adjust width based on header length or a minimum
  }));

  // Generate a buffer and save the file
  const buffer = await workbook.xlsx.writeBuffer();
  saveAs(new Blob([buffer], { type: "application/octet-stream" }), "exported_data.xlsx");
