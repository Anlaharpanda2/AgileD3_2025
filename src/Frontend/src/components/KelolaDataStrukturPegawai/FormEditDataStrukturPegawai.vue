 ...
 px-6 py-4 sm:px-8 sm:py-6 rounded-t-xl">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h2 class="text-xl sm:text-2xl font-bold text-white">Edit Data StrukturPegawai</h2>
                <p class="text-pink-100 text-xs sm:text-sm">Perbarui informasi struktur pegawai</p>
              </div>
            </div>
            <button
              @click="$emit('close')"
...
const submitForm = () => {
  formRef.value.validate(async (valid: boolean) => {
    ...
    try {
      ...
      const response = await api.put(`/kelola/struktur-pegawai/${form.id}`, payload);
        ElNotification({
          title: 'Berhasil',
