 ...
 px-6 py-4 sm:px-8 sm:py-6 rounded-t-xl">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h2 class="text-xl sm:text-2xl font-bold text-white">Tambah Data StrukturPegawai</h2>
                <p class="text-pink-100 text-xs sm:text-sm">Isi form untuk menambahkan data struktur pegawai baru</p>
              </div>
            </div>
            <button
              @click="$emit('close')"
          ...
          >
            ...
            <div class="bg-gray-50 rounded-xl p-5 sm:p-6 border border-gray-100 shadow-sm">
              <div class="flex items-center space-x-3 mb-5">
                ...
                <h3 class="text-base sm:text-lg font-semibold text-gray-800">Informasi StrukturPegawai</h3>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
...
import { reactive, ref } from 'vue'; // Removed 'watch' and 'onMounted' as they are not used in FormTambahDataStrukturPegawai
import { ElNotification } from 'element-plus';
import api from '../../api.js'; // Pastikan path ini benar

// Removed props as FormTambahDataStrukturPegawai does not use initialData
const emit = defineEmits(['close']);

const formRef = ref(null);
const statusOptions = ['kawin', 'lajang', 'janda'];

// Form fields definition (copied from FormTambahDataStrukturPegawai.vue)
const fields = [
...
// Reactive form state (copied from FormTambahDataStrukturPegawai.vue)
const form = reactive({
  nik: '',
  nama: '',
  jenis_bimtek: '',
...
// Form validation rules (copied from FormTambahDataStrukturPegawai.vue)
const rules = {
...
// Helper function to capitalize strings (copied from FormTambahDataStrukturPegawai.vue)
const capitalize = (s: string) => s.charAt(0).toUpperCase() + s.slice(1);

// Form submission logic (copied from FormTambahDataStrukturPegawai.vue)
const submitForm = () => {
  formRef.value.validate(async (valid: boolean) => {
    ...
    try {
      ...
      const response = await api.post('/kelola/struktur-pegawai', payload); // This is the POST request for adding data
        ElNotification({
          title: 'Berhasil',
