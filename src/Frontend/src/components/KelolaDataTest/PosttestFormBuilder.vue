<template>
  <div class="p-4">
    <el-card class="shadow-lg rounded-lg bg-white">
      ...
      <el-form :model="posttestForm" label-position="top" class="space-y-6">
        <el-form-item label="Judul Posttest" class="mb-4">
          <el-input v-model="posttestForm.title" placeholder="Masukkan judul posttest" class="w-full rounded-md border-gray-300 focus:border-pink-500 focus:ring focus:ring-pink-200 focus:ring-opacity-50" />
        </el-form-item>
        <el-form-item label="Deskripsi" class="mb-4">
          <el-input v-model="posttestForm.description" type="textarea" autosize placeholder="Masukkan deskripsi" class="w-full rounded-md border-gray-300 focus:border-pink-500 focus:ring focus:ring-pink-200 focus:ring-opacity-50" />
        </el-form-item>

        <h3 class="text-xl font-semibold mt-8 mb-4 text-gray-800">Daftar Soal</h3>
        <div v-if="posttestForm.questions.length === 0" class="text-gray-500 mb-4 p-4 border border-dashed border-gray-300 rounded-md text-center">
          Belum ada soal ditambahkan. Klik "Tambah Soal" untuk memulai.
        </div>
        <div v-for="(question, index) in posttestForm.questions" :key="question.order || index" class="mb-6">
        ...
        <div class="mt-8 flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-4">
          <el-button type="success" @click="savePosttest" class="w-full md:w-auto bg-green-500 text-white hover:bg-green-600 border-none rounded-md transition-all duration-300 ease-in-out">
