...
async function ExportWord(title: string, showLogo1: boolean, showLogo2: boolean) {
  ...
  const rows = [
    new TableRow({
      children: exportColumns.map(col =>
        new TableCell({
          children: [new Paragraph({ children: [new TextRun({ text: col.label, size: 16 })], alignment: AlignmentType.CENTER })]
        })
      )
    }),
    ...props.data.map(row =>
      ...
      new Table({
        ...
        rows: [
          new TableRow({
            children: [
              new TableCell({
                children: showLogo1 && logoLeftBuffer ? [new Paragraph({ children: [new ImageRun({ data: logoLeftBuffer, transformation: { width: 60, height: 60 }, type: "png" })], alignment: AlignmentType.CENTER })] : [new Paragraph({ text: "" })],
              ...
              new TableCell({
                children: showLogo2 && logoRightBuffer ? [new Paragraph({ children: [new ImageRun({ data: logoRightBuffer, transformation: { width: 60, height: 60 }, type: "png" })], alignment: AlignmentType.CENTER })] : [new Paragraph({ text: "" })],
                verticalAlign: "center",
                width: { size: 20, type: WidthType.PERCENTAGE },
              }),
            ],
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [new Paragraph({ text: "", spacing: { after: 200 } }), new Table({ rows })]
    }],
  })

  const buffer = await Packer.toBlob(doc)
  saveAs(buffer, 'struktur_pegawai.docx')
}
...
async function ExportPdf(title: string, showLogo1: boolean, showLogo2: boolean) {
  ...
  const [logo1, logo2] = await Promise.all([
    showLogo1 ? loadImage('/export/logo1.png') : null,
    showLogo2 ? loadImage('/export/logo2.png') : null
  ])

  if (logo1) doc.addImage(logo1.dataUrl, 'PNG', margin, startY, 60, 60)
  if (logo2) doc.addImage(logo2.dataUrl, 'PNG', pageWidth - margin - 60, startY, 60, 60)
  ...
  doc.text("Jalan Rasuna Said No. 74 Telp. (0751) 7053781 Fax. (0751) 7053781 Padang 25114", pageWidth / 2, startY + 35, { align: 'center' })

  doc.setLineWidth(2)
  doc.line(margin, startY + 50, pageWidth - margin, startY + 50)

  autoTable(doc, {
    head: [exportColumns.map(c => c.label)],
    body: props.data.map(r => exportColumns.map(c => r[c.key] ?? '')),
    startY: startY + 70,
    ...
    didDrawPage: () => {
      doc.setFontSize(fontSize8)
      doc.text(getCurrentUrl(), margin, pageHeight - 20)
      doc.text(`Dicetak pada: ${getCurrentDateTime()}`, pageWidth - margin, pageHeight - 20, { align: 'right' })
    }
  })

  doc.save('struktur_pegawai.pdf')
}
...
function ExportExcel() {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Struktur Pegawai")
  const wbout = XLSX.write(workbook, { bookType: "xlsx", type: "array" })
  saveAs(new Blob([wbout], { type: "application/octet-stream" }), "struktur_pegawai.xlsx")
}
...
<template>
  <div class="overlay" @click.self="$emit('close')">
    <div class="popup">
      ...
      <h2 class="title">Export Struktur Pegawai</h2>
