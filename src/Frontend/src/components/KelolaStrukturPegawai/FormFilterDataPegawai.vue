  ...
  <Transition name="dialog-fade">
    ...
    >
      ...
      <div class="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl shadow-gray-500/10 overflow-hidden transform transition-all duration-300 max-h-[90vh] flex flex-col">
        ...
        <div class="relative bg-gradient-to-r from-pink-50 to-pink-50 border-b border-gray-100">
...
const getColumnIcon = (column: string) => {
  ...
  if (col.includes('job') || col.includes('pekerjaan') || col.includes('kerja') || col.includes('jabatan')) return IconBriefcase;
  if (col.includes('company') || col.includes('perusahaan')) return IconBuilding;
  if (col.includes('position') || col.includes('posisi')) return IconIdCard;
  if (col.includes('status')) return IconInfo;
  if (col.includes('id')) return IconHash;
...
.bg-gradient-to-r.from-pink-600.to-pink-600:hover {
