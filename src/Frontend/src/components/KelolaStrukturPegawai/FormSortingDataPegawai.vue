    ...
    >
      ...
      >
        <div class="bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">
          <!-- Header Section -->
          <div class="relative bg-gradient-to-r from-pink-50 to-pink-50 px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100">
                ...
                <div class="space-y-2 sm:space-y-3">
                  ...
                  <label class="flex items-center p-3 sm:p-4 border-2 border-gray-100 rounded-xl cursor-pointer hover:border-pink-200 hover:bg-pink-50/30 transition-all duration-200 group touch-manipulation">
                    ...
                    <div class="flex-1">
                      <div class="flex items-center space-x-2 sm:space-x-3">
                        <div class="flex-shrink-0">
                          <div class="w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-br from-pink-100 to-pink-100 rounded-full flex items-center justify-center group-hover:from-pink-200 group-hover:to-pink-200 transition-all duration-200">
                            <svg
                              class="w-3 h-3 sm:w-4 sm:h-4 text-pink-600"
                              fill="none"
                              stroke="currentColor"
                              viewBox="0 0 24 24"
                            >
          ...
          <div class="bg-gray-50/50 px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-100">
            <div class="flex flex-col-reverse sm:flex-row space-y-reverse space-y-2 sm:space-y-0 sm:space-x-3 justify-end">
              ...
              <button
                class="w-full sm:w-auto px-4 sm:px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-pink-500 to-pink-500 rounded-lg hover:from-pink-600 hover:to-pink-600 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 touch-manipulation"
                @click="applySort"
              >
                Terapkan
              </button>
...
:deep(.modern-select .el-input__wrapper:hover) {
  border-color: #93c5fd; /* pink-300 */
}

:deep(.modern-select .el-input__wrapper.is-focused) {
  border-color: #60a5fa; /* pink-400 */
  box-shadow: 0 0 0 4px rgba(191, 219, 254, 0.5); /* ring-4 ring-pink-100 */
}
...
:deep(.el-radio__input.is-checked .el-radio__inner) {
  background-color: #3b82f6; /* pink-500 */
...
:deep(.el-select-dropdown__item.hover) {
  background-color: #eff6ff; /* pink-50 */
  color: #2563eb; /* pink-700 */
}

:deep(.el-select-dropdown__item.selected) {
  background-color: #dbeafe; /* pink-100 */
  color: #1e40af; /* pink-800 */
  font-weight: 600;
}
