<template>
  <SimpleLayout>
    <div class="min-h-screen bg-gray-100 font-sans relative">
      <button @click="fillWithDemoData" class="demo-button">Demo Pelatihan</button>
      <!-- Header -->
...
function fillWithDemoData() {
  form.status = 'lajang';
  form.pendidikan = 'SMK RPL';
  form.nomor_telefon = '089876543210';
  form.alamat = 'Jl. Demo No. 123, Kota Demo';
  form.jenis_bimtek = 'Bimtek Kewirausahaan Digital';
  form.jenis_usaha = 'Kuliner Kekinian';
  form.penghasilan_perbulan = '3000000';
}
...
const resetPage = () => {
  const savedNIK = localStorage.getItem('savedNIK');
  if (savedNIK) {
    router.push({ path: `/status-pelatihan` });
  } else {
    router.push({ path: '/' });
  }
};
