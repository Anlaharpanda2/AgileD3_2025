  ...
  <div class="min-h-screen bg-gray-100 font-sans">
    ...
    <div class="bg-white px-6 py-4 sm:px-8 sm:py-6 shadow-sm border-b border-gray-200">
      <div class="flex items-center justify-between">
        <!-- Back Arrow -->
        <button @click="goBack" class="p-2 -ml-2 rounded-full hover:bg-gray-100 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-pink-500">
        ...
        <div class="flex items-center space-x-3 flex-grow ml-4">
          ...
          <div>
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800">Daftar Pelatihan</h2>
            <p class="text-gray-600 text-xs sm:text-sm">Lengkapi informasi untuk mendaftar pelatihan baru</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="p-6 sm:p-8 bg-white shadow-sm border-b border-gray-200" v-if="!isLoadingQuota && isQuotaAvailable && !isSubmitted">
      <div class="flex items-center justify-between space-x-2">
        <div v-for="(step, index) in steps" :key="index" class="flex-1 flex items-center">
          <div class="flex items-center w-full">
            ...
            >
              {{ index < 4 ? index + 1 : '' }}
              <svg v-if="index === 4 && isSubmitted" class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Connector Line -->
            <div v-if="index < steps.length - 1" class="flex-1 h-1 bg-gray-200 mx-2 rounded-full overflow-hidden">
              <div
...
onMounted(async () => {
  ...
  try {
    ...
    if (savedNik) {
      ...
      } else if (typeof cekNikResponse.exists === 'boolean') {
        // Fallback: Check if exists is directly on the response object (less common for Axios, but possible with custom API wrappers or if 'data' is omitted)
        existsInResponse = cekNikResponse.exists;
      }
