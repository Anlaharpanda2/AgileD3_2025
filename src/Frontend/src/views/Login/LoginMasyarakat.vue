...
import api from '../../api'; // <-- Ganti dengan path yang benar ke instance axios Anda

// --- State Management dengan Composition API ---
const nik = ref<string>('');
const nama = ref<string>('');
const showNik = ref<boolean>(false);
...
// --- Computed Property untuk Tipe Input ---
// Cara yang lebih bersih untuk mengubah tipe input daripada menggunakan `watch`.
const nikInputType = computed(() => (showNik.value ? 'text' : 'password'));
...
onMounted(() => {
  ...
  if (localStorage.getItem('isLoggedIn') === 'true') {
    // Di aplikasi nyata, Anda mungkin ingin memvalidasi token sebelum redirect
...
const handleLogin = async () => {
  ...
  if (!/^\d{16}$/.test(nik.value)) {
    errorMessage.value = 'NIK harus terdiri dari 16 angka.';
    return;
  }
  if (/\d/.test(nama.value)) {
    errorMessage.value = 'Nama tidak boleh mengandung angka.';
    return;
  }
  ...
  try {
    ...
    // --- PERBAIKAN ---
    // Memberi tahu TypeScript untuk memperlakukan respons sebagai tipe 'any'
    // karena instance API Anda sepertinya mengubah respons asli.
    const { id, tempat_tanggal_lahir } = response as any;

    // Pastikan data yang diterima tidak kosong sebelum melanjutkan
    if (id && tempat_tanggal_lahir) {
      // Simpan data ke localStorage
      localStorage.setItem('isLoggedIn', 'true');
      localStorage.setItem('role', 'masyarakat');
      localStorage.setItem('savedNIK', nik.value);
      localStorage.setItem('savedName', nama.value);
      localStorage.setItem('userId', id);
      localStorage.setItem('Ttl', tempat_tanggal_lahir);

      // Redirect setelah berhasil
      await router.push('/');
    } else {
      // Jika respons berhasil tetapi tidak ada id atau ttl
      errorMessage.value = 'Data login tidak lengkap dari server.';
    }

  } catch (error: any) {
    errorMessage.value = error.response?.data?.message || 'Login gagal. NIK atau Nama tidak cocok.';
  } finally {
      ...
      >
        ...
        >
          <div class="w-full max-w-[280px] lg:max-w-[250px]">
            <h2 class="text-center text-black text-2xl lg:text-xl font-bold font-['Poppins'] mb-2">MASUK</h2>
            ...
            <form @submit.prevent="handleLogin" class="space-y-4 lg:space-y-3">
              <div class="relative w-full h-[45px] lg:h-[35px]">
                <div class="absolute left-3 top-1/2 -translate-y-1/2">
                  <img class="w-5 h-5 lg:w-4 lg:h-4" src="https://c.animaapp.com/IJi6pJQY/img/frame-1.svg" alt="User icon">
                </div>
                <input
                  v-model="nama"
                  type="text"
              ...
              <div class="flex items-center pt-2 lg:pt-1">
                ...
                >
                <label for="show-nik" class="ml-2 text-sm lg:text-[8px] font-['Poppins'] text-gray-700">Tampilkan NIK</label>
              </div>
              ...
              <div v-if="errorMessage" class="!mt-4 bg-red-500 text-white text-sm lg:text-xs text-center p-3 lg:p-2 rounded-md">
                {{ errorMessage }}
              </div>
            </form>

            <p class="text-center text-sm lg:text-[9px] mt-6 lg:mt-4 font-['Roboto']">
              <span class="font-light text-black">Belum memiliki akun? </span>
              <a href="/daftar/masyarakat" class="font-bold text-[#fe3275] hover:underline">Daftar</a>
            </p>
            ...
            <div class="space-y-3 lg:space-y-2">
              <a href="/login/operator" class="flex items-center justify-center w-full h-[45px] lg:h-[35px] border border-gray-300 rounded-[10.67px] hover:bg-gray-100 hover:border-pink-400 transition group">
                <img src="https://c.animaapp.com/IJi6pJQY/img/manage-accounts-24dp-e8eaed-fill0-wght400-grad0-opsz24-1.svg" class="w-6 h-6 lg:w-5 lg:h-5 mr-4" alt="Operator icon">
              ...
              <a href="/login/pegawai" class="flex items-center justify-center w-full h-[45px] lg:h-[35px] border border-gray-300 rounded-[10.67px] hover:bg-gray-100 hover:border-pink-400 transition group">
                <img src="https://c.animaapp.com/IJi6pJQY/img/mask-group-1@2x.png" class="w-6 h-6 lg:w-5 lg:h-5 mr-4" alt="Pegawai icon">
                <p class="text-sm lg:text-[8px] text-gray-800 font-['Poppins'] group-hover:text-pink-600">Masuk Sebagai <span class="font-bold">Pegawai</span></p>
              </a>
            </div>
          </div>
        </div>
