    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
      ...
      <div class="mb-6 sm:mb-8">
        ...
        <p class="text-gray-600 text-sm sm:text-base">
          Kelola dan pantau semua data berita yang telah dihapus
        </p>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sm:block hidden">
        <div class="overflow-x-auto">
          ...
          >
            ...
            >
              <template #default="{ row }">
                <div class="text-gray-600 text-sm full-width-content line-clamp-2">
                  {{ row.isi }}
                </div>
              </template>
            </el-table-column>
...
function toggleSelection(row: Berita) {
...
async function onDeleteClick(row: Berita) {
  ElMessageBox.confirm(
    `Apakah Anda yakin ingin menghapus data ${row.judul} secara permanen?`,
...
async function onRestoreClick(row: Berita) {
  ElMessageBox.confirm(
    `Apakah Anda yakin ingin memulihkan data ${row.judul}?`,
...
async function onMassDeletePermanentClick() {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      await api.delete('/kelola/berita/permanent', {
...
async function onMassRestoreClick() {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      await api.put('/kelola/berita/restore', {
        ids: idsToRestore,
      });
