<template>
  <Layout>
    <div class="min-h-screen bg-gray-50">
      ...
      <!-- Error State -->
      <div v-else-if="error" class="max-w-6xl mx-auto px-6 py-12">
        <div class="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
          ...
          <h3 class="text-xl font-medium text-red-800 mb-3">Terjadi Kesalahan</h3>
          <p class="text-red-600 mb-6">{{ error }}</p>
          <button @click="fetchBeritaDetail" class="bg-pink-500 hover:bg-pink-600 text-white px-6 py-3 rounded-lg transition-colors duration-200">
            Coba Lagi
          </button>
        </div>
      </div>
      ...
      <!-- Content -->
      <div v-else-if="berita" class="max-w-7xl mx-auto px-6 py-8">
        <!-- Header Section -->
        <div class="bg-gradient-to-r from-pink-500 to-pink-600 rounded-xl shadow-lg overflow-hidden mb-8">
          <div class="px-8 py-10 text-white">
            ...
            <div class="flex-1 min-w-0"> <div class="flex items-center space-x-3 mb-4">
<button @click="goBack" class="inline-flex items-center px-4 py-2
  bg-gray-100 text-gray-700 rounded-lg
  shadow-sm hover:bg-gray-200 transition-all duration-200
  font-medium text-sm">
  <el-icon class="mr-1.5" :size="18" color="#6B7280"> <ArrowLeft />
  </el-icon>
  Kembali
</button>
                </div>
                <h1 class="text-4xl font-bold mb-6 break-words">{{ berita.judul }}</h1> <div class="flex items-center space-x-8 text-pink-100">
                ...
                <div v-if="berita.updated_at !== berita.created_at" class="flex items-center">
                    <svg class="w-6 h-6 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div class="lg:col-span-3">
            <!-- Featured Image -->
            <div class="mb-8">
            <div v-if="berita.foto" class="relative group rounded-xl overflow-hidden shadow-lg">
                <img 
                :src="berita.foto" 
                :alt="berita.judul"
            ...
            <div v-else class="w-full h-96 bg-gray-200 rounded-xl shadow-lg flex items-center justify-center">
            ...
            <div class="bg-white rounded-xl shadow-lg p-8 mb-8">
              ...
              <div class="prose prose-lg max-w-none">
                <div class="text-gray-700 leading-relaxed text-lg text-content-wrapper whitespace-pre-line">{{ berita.isi }}</div>
              </div>
            </div>
                ...
                <div class="space-y-4">
                  ...
                  <div class="flex justify-between items-center py-3 border-b border-gray-100">
                    ...
                    <div class="text-right">
                      <div class="font-semibold text-gray-800">{{ formatDate(berita.updated_at) }}</div>
                      <div class="text-sm text-gray-500">{{ formatTime(berita.updated_at) }}</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="lg:col-span-1">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 sticky top-8">
              ...
              <div v-if="loadingRecommended" class="flex justify-center py-8">
                <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-pink-500"></div>
              </div>
              
              <!-- List rekomendasi berita -->
              <div v-else-if="recommendedNews.length > 0" class="space-y-4">
                <div 
                ...
                >
                  <div class="flex space-x-3">
                    <div v-if="news.foto" class="flex-shrink-0">
                    <img 
                        :src="news.foto" 
                        :alt="news.judul"
        ...
        <div class="bg-white rounded-xl shadow-lg mt-8 px-8 py-6">
          <div class="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
            <div class="flex items-center space-x-4">
              <button @click="shareBerita" class="inline-flex items-center px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors duration-200">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.367 2.684 3 3 0 00-5.367-2.684z"></path>
                </svg>
                Bagikan
              </button>
              <button @click="printBerita" class="inline-flex items-center px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors duration-200">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-center space-x-4">
              <button @click="editBerita" class="inline-flex items-center px-8 py-3 bg-pink-500 hover:bg-pink-600 text-white rounded-lg transition-colors duration-200 font-semibold">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                </svg>
                Edit Berita
              </button>
              <button @click="goBack" class="inline-flex items-center px-8 py-3 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition-colors duration-200 font-semibold">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Image Modal -->
        <div v-if="showImageModal" @click="closeImageModal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div class="relative max-w-5xl max-h-full">
            <img :src="berita.foto" :alt="berita.judul" class="max-w-full max-h-full object-contain rounded-lg">
            <button @click="closeImageModal" class="absolute top-4 right-4 bg-white bg-opacity-20 hover:bg-opacity-30 text-white p-3 rounded-full">
