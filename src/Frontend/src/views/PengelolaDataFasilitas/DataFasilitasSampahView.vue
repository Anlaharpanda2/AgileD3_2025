  ...
  <div class="mb-4">
    ...
    >
      ...
      <div class="mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">Data Fasilitas Sampah</h1>
        <p class="text-gray-600 text-sm sm:text-base">Kelola dan pantau semua data fasilitas yang telah dihapus</p>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 hover:shadow-md transition-shadow">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-xs sm:text-sm font-medium text-gray-600">Total Data</p>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sm:block hidden">
        <div class="overflow-x-auto">
          ...
          >
            ...
            <el-table-column prop="nama_fasilitas" label="Nama Fasilitas" min-width="150">
            ...
            <el-table-column prop="kategori" label="Kategori" min-width="120">
              <template #default="{ row }">
            ...
            <el-table-column prop="deskripsi" label="Deskripsi" min-width="160">
              <template #default="{ row }">
                <div class="text-gray-600 text-sm full-width-content">
                  {{ row.deskripsi }}
                </div>
            ...
            <el-table-column prop="jumlah" label="Jumlah" min-width="120">
              <template #default="{ row }">
                <div class="text-gray-600 text-sm full-width-content">
                  {{ row.jumlah }}
                </div>
        ...
        >
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-2">
              ...
              />
              <div>
                <p class="font-medium text-gray-900 text-sm">{{ row.nama_fasilitas }}</p>
                <p class="text-xs text-gray-600">{{ row.kategori }}</p>
              </div>
            </div>
...
const tableData = ref<Fasilitas[]>([]);
const selected = ref<Fasilitas[]>([]);
const search = ref("");
...
const editData = ref<Fasilitas | null>(null);
...
const visibleColumns = computed(() => {
  ...
  return allColumns.value.filter(col => ['nama_fasilitas', 'kode_fasilitas', 'jenis_fasilitas', 'lokasi'].includes(col.key));
});
...
const filterableColumns = computed(() => {
  ...
  const exclude = ['id', 'created_at', 'updated_at', 'deleted_at', 'kode_fasilitas', 'jenis_fasilitas', 'tanggal_pengadaan', 'lokasi', 'kondisi'];
...
const filteredData = computed(() => {
  ...
  if (search.value) {
    data = data.filter(item =>
      item.nama_fasilitas.toLowerCase().includes(search.value.toLowerCase())
    );
  }
...
function onSelectionChange(rows: Fasilitas[]) {
  selected.value = rows;
}

function toggleSelection(row: Fasilitas) {
...
async function fetchData() {
  try {
    const res = await api.get('/kelola/fasilitas/trash');
...
async function onDeleteClick(row: Fasilitas) {
  ElMessageBox.confirm(
    `Apakah Anda yakin ingin menghapus data ${row.nama_fasilitas} secara permanen?`,
    ...
    try {
          await api.delete(`/kelola/fasilitas/permanent/${row.id}`);
      ElMessage({
        type: 'success',
...
async function onRestoreClick(row: Fasilitas) {
  ElMessageBox.confirm(
    `Apakah Anda yakin ingin memulihkan data ${row.nama_fasilitas}?`,
    ...
    try {
      await api.put(`/kelola/fasilitas/restore/${row.id}`);
      ElMessage({
        type: 'success',
...
async function onMassDeletePermanentClick() {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      await api.delete('/kelola/fasilitas/permanent', { data: { ids: idsToDelete } });
...
async function onMassRestoreClick() {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      await api.put('/kelola/fasilitas/restore', { ids: idsToRestore });
