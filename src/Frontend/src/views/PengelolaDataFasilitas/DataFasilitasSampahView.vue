    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
      ...
      <div class="mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">
          Data Fasilitas Sampah
        </h1>
        <p class="text-gray-600 text-sm sm:text-base">
          Kelola dan pantau semua data fasilitas yang telah dihapus
        </p>
      </div>
...
const filteredData = computed(() => {
  ...
  for (const key in activeFilters.value) {
    ...
    if (filterValue !== null && filterValue !== '') {
      data = data.filter(item => {
        const itemValue = item[key as keyof Fasilitas];
...
async function fetchData() {
  try {
    ...
    tableData.value = (Array.isArray(res) ? res : res.data || []).sort((a: Fasilitas, b: Fasilitas) => {
