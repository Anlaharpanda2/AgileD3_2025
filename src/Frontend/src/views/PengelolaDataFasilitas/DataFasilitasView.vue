  ...
  <Layout2>
    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
      ...
      <div class="mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">Data Fasilitas</h1>
        <p class="text-gray-600 text-sm sm:text-base">Kelola dan pantau semua data fasilitas dalam satu tempat</p>
      </div>
...
const goToDetail = (row: any, column: any, event: MouseEvent) => {
  ...
  // router.push({ name: 'DetailFasilitas', params: { id: row.id } }); // Assuming you have a detail route for fasilitas
};

// Interface for Fasilitas data structure
interface Fasilitas {
  id: number;
...
const filteredData = computed(() => {
  ...
  // Apply search filter based on 'nama_fasilitas'
  if (search.value) {
...
async function onMassDeleteClick() {
  ...
  try {
    const ids = selected.value.map(p => p.id); // Get IDs of selected fasilitas
    await api.delete('/kelola/fasilitas', {
...
async function goToTrash() {
  router.push('/data/fasilitas/sampah');
}

// Handles deletion of a single row
async function onDelete(row: Fasilitas) {
  loading.value = true; // Show loading indicator
  try {
    await api.delete(`/kelola/fasilitas/${row.id}`); // Delete by ID
    await fetchData(); // Refresh data after deletion
    ElNotification({
      title: 'Berhasil',
...
async function fetchData() {
  ...
  try {
    ...
    let fetchedData: Fasilitas[] = Array.isArray(res) ? res : res.data || [];
