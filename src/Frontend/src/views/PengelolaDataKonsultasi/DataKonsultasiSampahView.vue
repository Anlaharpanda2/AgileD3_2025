    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
      ...
      <div class="mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">
          Data Konsultasi Sampah
        </h1>
        <p class="text-gray-600 text-sm sm:text-base">
          Kelola dan pantau semua data konsultasi yang telah dihapus
        </p>
      </div>
...
const visibleColumns = computed(() => {
  ...
  return allColumns.value.filter(col => ['nama_pelapor', 'nama_korban', 'kasus', 'status'].includes(col.key));
});
...
const filterableColumns = computed(() => {
  ...
  const exclude = ['id', 'created_at', 'updated_at', 'deleted_at', 'lampiran'];
...
async function onMassDeletePermanentClick() {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      await api.post('/kelola/konsultasi/permanent', {
        ids: idsToDelete,
      });
...
async function onMassRestoreClick() {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      await api.put('/kelola/konsultasi/restore', {
        ids: idsToRestore,
      });
