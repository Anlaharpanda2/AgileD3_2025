<template>
  <Layout2>
    ...
    <FormEditDataPanitia
      ...
      class="sm:full-screen"
    />
    <FormExportDataPanitia
      v-if="showExport"
      :data="pagedData"
      @close="showExport = false"
      class="sm:full-screen"
    />
    <FormImportDataPanitia
      v-if="showImport"
    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
      ...
      <div class="mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">Data Panitia</h1>
        <p class="text-gray-600 text-sm sm:text-base">Kelola dan pantau semua data panitia dalam satu tempat</p>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 hover:shadow-md transition-shadow">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-xs sm:text-sm font-medium text-gray-600">Total Panitia</p>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sm:block hidden">
        <div class="overflow-x-auto">
          ...
          >
            ...
            <el-table-column prop="nama_panitia" label="Nama Panitia" min-width="150">
            ...
            <el-table-column prop="jabatan" label="Jabatan" min-width="140">
              <template #default="{ row }">
            ...
            <el-table-column prop="no_hp" label="No. HP" min-width="160">
              <template #default="{ row }">
                <div class="text-gray-600 text-sm full-width-content">
                  {{ row.no_hp }}
                </div>
            ...
            <el-table-column label="Foto" min-width="120">
              <template #default="{ row }">
                <img :src="getImageUrl(row)" alt="Foto Panitia" class="w-16 h-16 object-cover rounded-full" />
              </template>
            </el-table-column>
        ...
        >
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-2">
              ...
              />
              <div>
                <p class="font-medium text-gray-900 text-sm">{{ row.nama_panitia }}</p>
                <p class="text-xs text-gray-600">{{ row.email }}</p>
              </div>
            </div>
          ...
          <div class="text-xs text-gray-600 mb-2">
            <p>{{ row.no_hp }}</p>
            <img :src="getImageUrl(row)" alt="Foto Panitia" class="w-16 h-16 object-cover rounded-full mt-2" />
          </div>
...
import FormExportDataPanitia from "../../components/KelolaDataPanitia/FormExportDataPanitia.vue";
import FormImportDataPanitia from "../../components/KelolaDataPanitia/FormImportDataPanitia.vue";
...
const goToDetail = (row: any, column: any, event: MouseEvent) => {
  ...
  // Assuming you have a detail route for panitia
  // router.push({ name: 'DetailPanitia', params: { id: row.id } });
};

// Interface for Panitia data structure
interface Panitia {
  id: number;
  email: string;
  jabatan: string;
  nama_panitia: string;
  no_hp: string;
  foto: string; // Ini mungkin nama file saja
...
function getImageUrl(row: Panitia): string {
  ...
  if (row.foto) return `/storage/${row.foto}`; // Fallback to old path if only file name
  return '/placeholder.jpg'; // fallback jika kosong
}
...
const filteredData = computed(() => {
  ...
  // Apply search filter based on 'nama_panitia'
  if (search.value) {
...
async function onMassDeleteClick() {
  ...
  try {
    const ids = selected.value.map(p => p.id); // Get IDs of selected panitia
    await api.delete('/kelola/panitia', {
      data: { ids } // Send IDs in the request body for mass delete
    });
...
async function goToTrash() {
  window.location.href = '/data/panitia/sampah';
}

// Handles deletion of a single row
async function onDelete(row: Panitia) {
  loading.value = true; // Show loading indicator
  try {
    await api.delete(`/kelola/panitia/${row.id}`); // Delete by ID
    await fetchData(); // Refresh data after deletion
    ElNotification({
      title: 'Berhasil',
...
async function fetchData() {
  ...
  try {
    const res = await api.get('/kelola/panitia');
    let fetchedData: Panitia[] = Array.isArray(res) ? res : res.data || [];
