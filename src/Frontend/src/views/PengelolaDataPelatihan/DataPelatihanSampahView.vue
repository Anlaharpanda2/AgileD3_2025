    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
      ...
      <div class="mb-6 sm:mb-8">
        ...
        <p class="text-gray-600 text-sm sm:text-base">
          Kelola dan pantau semua data pelatihan yang telah dihapus
        </p>
      </div>
...
const filteredData = computed(() => {
  ...
  for (const key in activeFilters.value) {
    ...
    if (filterValue !== null && filterValue !== '') {
      data = data.filter(item => {
        const itemValue = item[key as keyof Peserta];
...
async function onRestoreClick(row: Peserta) {
  ElMessageBox.confirm(
...
async function onMassDeletePermanentClick() {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      await api.post('/kelola/pelatihan/permanent', {
        niks: niksToDelete,
      });
...
async function onMassRestoreClick() {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      await api.put('/kelola/pelatihan/restore', {
        niks: niksToRestore,
      });
