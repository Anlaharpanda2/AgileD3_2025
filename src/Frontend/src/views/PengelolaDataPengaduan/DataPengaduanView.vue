  ...
  <Layout2>
    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
      ...
      <div class="mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">Data Pengaduan</h1>
        <p class="text-gray-600 text-sm sm:text-base">Kelola dan pantau semua data pengaduan dalam satu tempat</p>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 hover:shadow-md transition-shadow">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-xs sm:text-sm font-medium text-gray-600">Total Pengaduan</p>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sm:block hidden">
        <div class="overflow-x-auto">
          ...
          >
            ...
            <el-table-column prop="kode_pengaduan" label="Kode Pengaduan" min-width="150">
            ...
            <el-table-column prop="nama_pelapor" label="Nama Pelapor" min-width="120">
              <template #default="{ row }">
            ...
            <el-table-column prop="nama_korban" label="Nama Korban" min-width="140">
              <template #default="{ row }">
            ...
            <el-table-column prop="kasus" label="Kasus" min-width="160">
              <template #default="{ row }">
                <div class="text-gray-600 text-sm full-width-content">
                  {{ row.kasus }}
                </div>
        ...
        >
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-2">
              ...
              />
              <div>
                <p class="font-medium text-gray-900 text-sm">{{ row.nama_pelapor }}</p>
                <p class="text-xs text-gray-600">{{ row.kode_pengaduan }}</p>
              </div>
            </div>
...
import FormImportDataPengaduan from "../../components/KelolaDataPengaduan/FormImportDataPengaduan.vue";
import FormEditDataPengaduan from "../../components/KelolaDataPengaduan/FormEditDataPengaduan.vue";
...
const goToDetail = (row: any, column: any, event: MouseEvent) => {
  ...
  router.push(`/pengaduan/status/${row.id}`);
};
...
function getLampiranUrl(row: Pengaduan): string {
  if (row.lampiran_url) {
    let url = row.lampiran_url;
  ...
  if (row.lampiran) return `/storage/${row.lampiran}`; // Fallback ke path lama jika hanya nama file
...
const filterableColumns = computed(() => {
  ...
  const exclude = ['id', 'created_at', 'updated_at', 'deleted_at', 'lampiran', 'lampiran_url', 'waktu_kejadian', 'no_hp', 'alamat', 'saksi', 'deskripsi'];
...
async function onMassDeleteClick() {
  ...
  try {
    const ids = selected.value.map(p => p.id); // Get IDs of selected pengaduan
    await api.delete('/kelola/pengaduan', {
...
async function goToTrash() {
  window.location.href = '/data/pengaduan/sampah';
}
...
async function onDelete(row: Pengaduan) {
  loading.value = true; // Show loading indicator
  try {
    await api.delete(`/kelola/pengaduan/${row.id}`); // Delete by ID
    await fetchData(); // Refresh data after deletion
    ElNotification({
      title: 'Berhasil',
...
async function fetchData() {
  ...
  try {
    const res = await api.get('/kelola/pengaduan');
