    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
      ...
      <div class="mb-6 sm:mb-8">
        ...
        <p class="text-gray-600 text-sm sm:text-base">
          Kelola dan pantau semua data pegawai dalam satu tempat
        </p>
      </div>
          ...
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
              ...
              <div class="relative sm:flex-1 sm:max-w-[100px]">
                <div
                  class="relative flex items-center gap-[6px] cursor-pointer select-none bg-[#EC4899] text-white rounded-[6px] px-[10px]
                  "
                >
                  <button
                    id="itemsPerPageBtn"
            ...
            <div class="flex flex-col sm:flex-row gap-3">
              <div class="relative sm:flex sm:gap-3 mobile-actions">
                ...
                <button
                  class="inline-flex items-center px-6 py-2.5 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-lg text-sm font-medium hover:from-pink-600 hover:to-rose-600 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 transform hover:scale-105 transition-all duration-200 shadow-md w-full sm:w-auto touch-highlight"
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sm:block hidden">
        <div class="overflow-x-auto">
          ...
          >
            ...
            >
              <template #default="{ row }">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-pink-100 text-pink-800 full-width-content">
                  {{ row.jabatan }}
                </span>
              </template>
            </el-table-column>
            ...
            >
              <template #default="{ row }">
                <span
                  :class="['status-badge', {
                    'bg-green-100 text-green-800': row.status === 'aktif',
                    'bg-red-100 text-red-800': row.status === 'nonaktif',
                  }]"
                >
                  {{ row.status }}
                </span>
            ...
            >
              <template #default="{ row }">
                <div class="flex items-center gap-2 full-width-content">
                  <button
                    class="w-8 h-8 flex items-center justify-center rounded-lg bg-pink-100 text-pink-600 hover:bg-pink-200 transition-colors"
                    title="Edit"
      ...
      <div class="block sm:hidden space-y-4">
        ...
        >
          ...
          <div class="flex gap-2">
            <button
              class="flex-1 py-2 bg-pink-100 text-pink-600 rounded-lg text-sm hover:bg-pink-200 transition-colors touch-highlight"
              @click.stop="openEdit(row)"
            >
              Edit
            </button>
...
import FormExportDataPegawai from "../../components/KelolaStrukturPegawai/FormExportDataPegawai.vue";
import FormImportDataPegawai from "../../components/KelolaStrukturPegawai/FormImportDataPegawai.vue";
import FormEditDataPegawai from "../../components/KelolaStrukturPegawai/FormEditDataPegawai.vue";
import FormTambahDataPegawai from "../../components/KelolaStrukturPegawai/FormTambahDataPegawai.vue";
import FormFilterDataPegawai from "../../components/KelolaStrukturPegawai/FormFilterDataPegawai.vue";
import FormSortingDataPegawai from "../../components/KelolaStrukturPegawai/FormSortingDataPegawai.vue";
...
const filteredData = computed(() => {
  ...
  for (const key in activeFilters.value) {
    ...
    if (filterValue !== null && filterValue !== '') {
      data = data.filter(item => {
        const itemValue = item[key as keyof Pegawai];
...
function toggleSelection(row: Pegawai) {
...
async function onMassDeleteClick() {
  ...
  try {
    ...
    await api.delete('/kelola/struktur-pegawai', {
      data: { ids }
    });
    await fetchData();
    ElNotification({
...
async function onDelete(row: Pegawai) {
  loading.value = true;
  try {
    await api.delete(`/kelola/struktur-pegawai/${row.id}`);
    await fetchData();
    ElNotification({
      title: 'Berhasil',
...
async function fetchData() {
  loading.value = true;
  try {
    const res = await api.get('/kelola/struktur-pegawai');
    let fetchedData: Pegawai[] = Array.isArray(res) ? res : res.data || [];
