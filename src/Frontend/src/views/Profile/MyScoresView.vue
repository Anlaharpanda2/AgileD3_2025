    ...
    <div class="container mx-auto p-4">
      <el-card class="box-card">
        ...
        <div v-if="isLoading" class="text-center text-blue-500 mt-8">
          <p class="text-lg">Memuat nilai...</p>
        </div>
        ...
        <div v-else-if="scoreData">
          <p class="text-lg mb-2">Nama: <span class="font-semibold">{{ scoreData.user_name }}</span></p>
          <p class="text-lg mb-2">NIK: <span class="font-semibold">{{ scoreData.user_nik }}</span></p>
          ...
          <div class="mt-4 flex gap-2">
            <el-button v-if="scoreData.pretest_score === null" type="primary" @click="$router.push({ name: 'TakePretest' })">Ikuti Pretest</el-button>
            <el-button v-if="scoreData.posttest_score === null" type="success" @click="$router.push({ name: 'TakePosttest' })">Ikuti Posttest</el-button>
          </div>
        </div>

        <div v-else class="text-center text-gray-500 mt-8">
          <p class="text-lg">Anda belum mengerjakan pretest atau posttest.</p>
          <div class="mt-4 flex gap-2">
            <el-button type="primary" @click="$router.push({ name: 'TakePretest' })">Ikuti Pretest</el-button>
            <el-button type="success" @click="$router.push({ name: 'TakePosttest' })">Ikuti Posttest</el-button>
          </div>
        </div>
        ...
        <div class="mt-8">
          <el-button type="primary" @click="$router.back()">Kembali</el-button>
        </div>
      </el-card>
    </div>
  </Layout2>
...
onMounted(async () => {
  ...
  try {
    ...
    if (err.response && err.response.status === 404) {
      ...
      error.value = `Gagal memuat nilai: ${err.message || 'Terjadi kesalahan tidak dikenal.'}`;
    }
    ElMessage.error(error.value);
    console.error(err);
  } finally {
