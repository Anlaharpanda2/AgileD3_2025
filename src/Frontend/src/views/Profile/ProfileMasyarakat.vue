    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-white">
      ...
      <div class="relative overflow-hidden bg-gradient-to-r from-pink-500 via-pink-400 to-pink-300">
        ...
        <div class="absolute inset-0">
          <div class="absolute top-0 left-0 w-72 h-72 bg-white opacity-10 rounded-full mix-blend-multiply filter blur-xl animate-pulse" />
          <div class="absolute top-0 right-0 w-72 h-72 bg-pink-200 opacity-20 rounded-full mix-blend-multiply filter blur-xl animate-pulse delay-1000" />
          <div class="absolute -bottom-8 left-20 w-72 h-72 bg-gray-200 opacity-15 rounded-full mix-blend-multiply filter blur-xl animate-pulse delay-2000" />
        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
          <template v-if="pesertaData">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                <div class="p-6 space-y-4">
                  ...
                  <div class="flex justify-between items-center">
                    ...
                    <span
                      ...
                      :class="getStatusClass(pesertaData.status)"
                    >
                      {{ pesertaData.status }}
                    </span>
                  </div>
              ...
              <!-- Informasi Sistem -->
              <div
                v-if="pesertaData.created_at || pesertaData.updated_at"
                class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-300 hover:scale-105"
              >
                <div class="bg-gradient-to-r from-pink-50 to-gray-50 px-6 py-4 border-b border-gray-100">
                  <h3 class="text-lg font-bold text-gray-500 flex items-center">
                    <div class="w-10 h-10 bg-gradient-to-br from-pink-400 to-pink-600 rounded-lg flex items-center justify-center mr-3">
          ...
          <template v-else>
            <div class="flex flex-col items-center justify-center min-h-96 bg-white rounded-3xl shadow-2xl border border-gray-100">
              <div class="relative">
                <div class="w-16 h-16 border-4 border-gray-200 border-t-pink-500 rounded-full animate-spin" />
                <div class="absolute inset-0 w-16 h-16 border-4 border-transparent border-t-pink-300 rounded-full animate-spin animate-reverse delay-150" />
              </div>
              ...
              <div class="mt-2 flex space-x-1">
                ...
                <div class="w-2 h-2 bg-pink-400 rounded-full animate-bounce delay-100" />
                <div class="w-2 h-2 bg-pink-400 rounded-full animate-bounce delay-200" />
              </div>
            </div>
          </template>
        </div>
...
import api from '../../api'
import DefaultSimpleLayout from "../../layouts/SimpleLayout.vue";
...
const uploadPhoto = async (event: Event) => {
  ...
  try {
    ...
    const errorMsg = (err instanceof AxiosError && err.response?.data?.message) || (err instanceof Error ? err.message : 'Gagal mengunggah foto.')
